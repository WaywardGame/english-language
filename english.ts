...
english.hint(HintType.DoodadPickup, "Doodads", "Doodads are considered objects attached to the ground like plants, piles of rocks, furnaces, campfires, etc. To collect them, you can do one of the following:<br />1. While facing the item, click or press the \"Actions\" hotkey, and select the \"Collect Object\" option.<br />2. Use an item with \"Digging\" such as a shovel.<br />3. Use an item with \"Carving\" such as a sharp rock. Using your bare hands with no tool (option 1) can sometimes harm you. Using a tool (option 2 and 3) will decrease the durability of the item. Some objects may require a \"Carving\" tool such as corpses to harvest from.");
...
english.hint(HintType.Bugs, "BUGS!", "Did you find an error? Would you kindly let us know about what happened, so that we may seek to stop this from happening to other players? You can do so by posting the issue in <a target=\"_blank\" href=\"http://steamcommunity.com/app/379210/discussions/1/\">Steam Discussions</a>.");
...
english.hint(HintType.Crafting, "Crafting", "Crafting is simple, but has many rules!<br /><br />When hovering over an item in the crafting window, you will see which items will be used in the craft through a highlighted border that will appear around items in your inventory, quickslot or equipment windows (priority is set in that order). You will also notice that you will use the first instance of a required item. Simply drag around the order of items in your inventory to use different items in the craft.<br /><br />Using items in a craft that have lower durability will affect the crafted item's durability. Items that have zero durability cannot be used in a craft. Additionally, using Remarkable, Exceptional or Legendary items in a craft increases your chances to craft such an item. Decayable items (such as food) also gain decay bonuses from using higher quality items in the craft.<br /><br />You can find new crafting recipes by: gathering the required items in your inventory and by finding Old Instructional Scrolls.");
